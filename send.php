...
if(!isset($_POST['fio']) and !isset($_POST['email'])){
    ?>
        <h1>Получить результаты на почту</h1>
    <form action="send.php" method="post">
        <label><font size="4">ФИО:</font></label>
        <input type="text" name="fio" required><br>
        <label><font size="4">Email:</font></label>
        <input type="text" name="email" required><br>
        <label><font size="4">Название теста:</font></label>
        <input type="text" name="test" required><br>
        <label><font size="4">Количество верных ответов:</font></label>
        <input type="text" name="corans" required><br>
    ...
    $corans = htmlspecialchars($corans);
    $fio = urldecode($fio);
    $email = urldecode($email);
    $test = urldecode($test);
    $corans = urldecode($corans);
    $fio = trim($fio);
    $email = trim($email);
    $test = trim($test);
    $corans = trim($corans);

    if (mail($email, "Результат теста", "ФИО:".$fio."\nНазвание теста: ".$test."\nКоличество правильных ответов: ".$corans,"From: bliznuk.aleksey@gmail.com \r\n")){
